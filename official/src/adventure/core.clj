...
(defn make [player]
  (let[location (player :location)
      ...
      (if (and (contains? invent :Quartus-II-Software/Hardware-Package) (contains? invent :Lab-Manual) (contains? invent :Monitor)
               (contains? invent :Pencil) (contains? invent :Laptop) (contains? invent :LED-Strips) (contains? invent :VGA-Cable)
               (contains? invent :FPGA) (contains? invent :Notebook))
          (update-in player [:inventory] #(conj % cont))
...
(defn submit [player]
  (let [location (player :location)
        invent (player :inventory)]
    (cond (not (contains? invent :final-project))
            (do (println "You didn't even make the final project. What are you doing with your life?") player)
          (not (= location :eceb))
            (do (println "Go to the ECEB to submit your project.") player)
          :else ((do (println "You successfully submitted your final project") player)
                 (java.lang.System/exit 0)))))
